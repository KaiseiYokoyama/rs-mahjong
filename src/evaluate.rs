...
use crate::groups::{Tiles, OpenSet, Set, Sets};
...
pub struct Evaluator {
    /// 確定している状況役(リーチ、ツモなど)
    pub situation: Vec<SituationYaku>,
    /// 採用されている役満
    pub adopted_yakuman_list: Vec<Yakuman>,
    /// 採用されている手役
    pub adopted_yaku_list: Vec<HandYaku>,
    /// ツモorロン
    pub draw: bool,
    /// 門前
    pub closed: bool,
...
impl Evaluator {
    pub fn new(prevalent_wind: Option<Tile>, seat_wind: Option<Tile>, dora: Vec<Tile>, ura_dora: Vec<Tile>) -> Self {
        let (prevalent_wind_cloned, seat_wind_cloned) = (prevalent_wind.clone(), seat_wind.clone());
        let adopted_yaku_list = {
            let nopoints = HandYaku::new("平和 / No-points hand", None,
                                         Box::new(|candidate: &Candidate| {
                                             if !candidate.closed() { return None; }
                                             match candidate {
                                                 Candidate::Ryanmen(_, fu, _) => {
                                                     if fu == &Fu(30) {
                                                         Some(Han(1))
                                                     } else { None }
                                                 }
                              ...
                              Box::new(move |candidate: &Candidate| {
                                  ...
                                  han += Han(candidate.node().open_sets.iter().filter(|set| {
                                      set.count(&Dragon::White.tile()) >= 3 || set.count(&Dragon::Green.tile()) >= 3 || set.count(&Dragon::Red.tile()) >= 3 ||
                                          if let Some(seat_wind) = seat_wind_cloned.clone() {
                                              set.count(&seat_wind) >= 3
                                          } else { false } ||
                                          if let Some(wind) = prevalent_wind_cloned.clone() {
                                              set.count(&wind) >= 3
                                          } else { false }
                                  }).count() as u32);
        ...
        let adopted_yakuman_list = {
            let thirteen_orphans =
                Yakuman::new("国士無双 / Thirteen orphans", Box::new(|_candidate: &Candidate, _original_tiles: &Vec<Tile>| {
            ...
            let thirteen_orphans_13_wait =
                Yakuman::new("国士無双一三面待ち / Thirteen orphans 13 wait", Box::new(|_candidate: &Candidate, _original_tiles: &Vec<Tile>| {
            ...
            let big_three_dragons =
                Yakuman::new("大三元 / Big three dragons", Box::new(|_candidate: &Candidate, _original_tiles: &Vec<Tile>| {
            ...
            let four_concealed_triplets =
                Yakuman::new("四暗刻 / Four concealed triplets", Box::new(|_candidate: &Candidate, _original_tiles: &Vec<Tile>| {
            ...
            let little_four_winds =
                Yakuman::new("小四喜 / Little four dragons", Box::new(|_candidate: &Candidate, _original_tiles: &Vec<Tile>| {
            ...
            let big_four_winds =
                Yakuman::new("大四喜 / Big four dragons", Box::new(|_candidate: &Candidate, _original_tiles: &Vec<Tile>| {
                    let dragons = vec![Wind::East.tile(), Wind::South.tile(), Wind::West.tile(), Wind::North.tile()];
                    if dragons.iter().all(|dragon| {
                        _candidate.node().sets.iter().any(|set| set.count(dragon) >= 3)
                            || _candidate.node().open_sets.iter().any(|set| set.count(dragon) >= 3)
            ...
            let all_honors =
                Yakuman::new("字一色 / All honors", Box::new(|_candidate: &Candidate, _original_tiles: &Vec<Tile>| {
            ...
            let all_terminals =
                Yakuman::new("清老頭 / All terminals", Box::new(|_candidate: &Candidate, _original_tiles: &Vec<Tile>| {
            ...
            let all_green =
                Yakuman::new("緑一色 / All green", Box::new(|_candidate: &Candidate, _original_tiles: &Vec<Tile>| {
            ...
            let nine_gates =
                Yakuman::new("九蓮宝燈 / Nine gates", Box::new(|_candidate: &Candidate, _original_tiles: &Vec<Tile>| {
        ...
        Self { situation: Vec::new(), adopted_yakuman_list, adopted_yaku_list, draw: false, closed: true, prevalent_wind, seat_wind, dora, ura_dora }
    }
}

impl Evaluator {
    pub fn evaluate(&self, candidates: Candidates) -> Option<Evaluated> {
        self.evaluate_all(candidates).last().cloned()
    }

    pub fn evaluate_all(&self, candidates: Candidates) -> Vec<Evaluated> {
        let mut scores: Vec<Evaluated> = candidates.candidates
            .iter().map(|c| self.evaluate_candidate(&candidates.original_hand, c)).collect();
        scores.sort_by(|a, b| a.score.score(false).cmp(&b.score.score(false)));
        scores
    }

    fn evaluate_yaku(&self, yaku: &Box<HandYaku>, yaku_list: &mut Vec<String>, candidate: &Candidate, han: &mut Han) {
        let rule = &yaku.rule;
    ...
    fn evaluate_yakuman(&self, yakuman: &Box<Yakuman>, yakuman_list: &mut Vec<String>, candidate: &Candidate, original_hand: &Vec<Tile>) -> u32 {
    ...
    fn evaluate_candidate(&self, original_hand: &Vec<Tile>, candidate: &Candidate) -> Evaluated {
        ...
        self.adopted_yakuman_list.iter().for_each(|yakuman| {
            ...
            if 0 != rule(candidate, &original_hand) {
                yakuman_list.push(yakuman.name());
                multiple += rule(candidate, &original_hand);
            } else {
                if let Some(ref yakuman) = yakuman.sub {
                    multiple += self.evaluate_yakuman(yakuman, &mut yakuman_list, candidate, original_hand);
                }
            }
        });
        if multiple != 0 {
            return Evaluated { score: Score::yakuman(multiple as u8), node: candidate.node().clone(), yaku_list: yakuman_list };
        }
        ...
        for adopted_yaku in &self.adopted_yaku_list {
            ...
            if let Some(han_) = rule(candidate) {
                ...
                if let Some(fu_rule) = &adopted_yaku.fu {
                    fu = Some(fu_rule(&self.draw));
                }
            } else {
                if let Some(ref yaku) = adopted_yaku.sub {
                    self.evaluate_yaku(yaku, &mut yaku_list, candidate, &mut han);
                }
            }
        }
        ...
        Evaluated { score: Score::new(han, fu), node: candidate.node().clone(), yaku_list }
    }
}
...
impl Candidates {
    pub fn new(original_hand: Vec<Tile>, candidates: Vec<Candidate>) -> Self {
        Candidates { original_hand, candidates }
    }

    pub fn from_vec(original_hand: Vec<Tile>, nodes: Vec<Node>, winning: Tile, draw: bool, prevalent_wind: Option<Tile>, seat_wind: Option<Tile>) -> Candidates {
        let mut candidates = Vec::new();
        nodes.iter().for_each(|node|
            candidates.append(&mut Candidate::from(&node, winning.clone(),
...
impl Candidate {
    fn from(node: &Node, winning: Tile, draw: bool, prevalent_wind: Option<Tile>, seat_wind: Option<Tile>) -> Vec<Candidate> {
        // 待ち候補
        ...
        wait_candidates.iter().for_each(|set| {
            match set {
                Set::Pair(_) => {
                    candidates.push(Candidate::Tanki(node.clone(), fu + Fu(2), winning.clone()));
                }
                _ => {
                    if set.is_flat() {
                        candidates.push(Candidate::Shanpon(node.clone(), fu, winning.clone()));
                    } else {
                        ...
                        if Some(&winning) == set.first() || Some(&winning) == set.last() {
                            if set.contains_yaotyu() && !winning.is_yaotyu() {
                                candidates.push(Candidate::Penchan(node.clone(), fu + Fu(2), winning.clone()));
                            } else {
                                candidates.push(Candidate::Ryanmen(node.clone(), fu, winning.clone()));
                            }
                        } else {
                            candidates.push(Candidate::Kanchan(node.clone(), fu + Fu(2), winning.clone()));
                        }
                    }
                }
            }
    ...
    fn winning(&self) -> Tile {
        match &self {
            Candidate::Ryanmen(_, _, winning) => winning,
            Candidate::Kanchan(_, _, winning) => winning,
            Candidate::Penchan(_, _, winning) => winning,
            Candidate::Tanki(_, _, winning) => winning,
            Candidate::Shanpon(_, _, winning) => winning,
        }.clone()
    }
    ...
    pub fn closed(&self) -> bool {
        match &self {
            Candidate::Ryanmen(node, _, _) => {
                node
            }
            Candidate::Kanchan(node, _, _) => {
                node
            }
            Candidate::Penchan(node, _, _) => {
                node
            }
            Candidate::Shanpon(node, _, _) => {
                node
            }
            Candidate::Tanki(node, _, _) => {
                node
            }
    ...
    pub fn fu(&self) -> Fu {
        match &self {
            Candidate::Ryanmen(_, fu, _) => fu,
            Candidate::Kanchan(_, fu, _) => fu,
            Candidate::Penchan(_, fu, _) => fu,
            Candidate::Tanki(_, fu, _) => fu,
            Candidate::Shanpon(_, fu, _) => fu,
        }.clone()
    }
}
...
impl Display for Candidate {
    fn fmt(&self, f: &mut Formatter) -> Result<(), Error> {
        match &self {
            Candidate::Ryanmen(node, fu, _) => {
                writeln!(f, "両面待ち: Ryanmen {} / {}", fu, node)
            }
            Candidate::Kanchan(node, fu, _) => {
                writeln!(f, "嵌張待ち: Kanchan {} / {}", fu, node)
            }
            Candidate::Penchan(node, fu, _) => {
                writeln!(f, "辺張待ち: Penchan {} / {}", fu, node)
            }
            Candidate::Shanpon(node, fu, _) => {
                writeln!(f, "双碰待ち: Shanpon {} / {}", fu, node)
            }
            Candidate::Tanki(node, fu, _) => {
